...
context('Cross-Device Elements Test', () => {
  ...
  describe('All Viewport Tests', () => {
    ...
    viewports.forEach(size => {
      ...
      describe(`Viewport verification tests on viewport: [${checkSizes(size)}]`, function () {
        it(`Displays logo`, async function () {
  ...
  describe('Laptop Viewport Tests (1200X700)', () => {
    ...
    describe(`shows sidebar for filtering shoes on viewport: [${checkSizes(size)}]`, function () {
      ...
      it('shows consistent number of items in type filter', function () {
        ...
        verifyElementLength(elements.typeFilterElements(), 4)
      });
      ...
      it('shows consistent number of items in colors filter', function () {
        ...
        verifyElementLength(elements.colorsFilterElements(), 5)
      });
      ...
      it('shows consistent number of items in brands filter', function () {
        ...
        verifyElementLength(elements.brandsFilterElements(), 5)
      });
      ...
      it('shows consistent number of items in price filter', function () {
        ...
        verifyElementLength(elements.priceFilterElements(), 4)
      });
    });

    describe(`shows shopping utilities on hover of items on viewport [${checkSizes(size)}]`, function () {
    ...
    it('shows search placeholder on the search field', function () {
      shouldInvokeAttribute(fetchTestTitle(this), elements.searchInput(),
        viewPortSize(size), 'Search over 10,000 shoes!');
      verifyAttributeInvocation(elements.searchInput(), 'Search over 10,000 shoes!');
    });
  ...
  describe('Tablet Viewport Tests (768 * 700)', () => {
    ...
    it(`shows email placeholder`, function () {
      shouldInvokeAttribute(fetchTestTitle(this), elements.keepInTouchEmail(),
        viewPortSize(size), 'Your email');
      verifyAttributeInvocation(elements.keepInTouchEmail(), 'Your email');
    });
  });
  ...
  describe('Tablet and iphone-x Viewport Tests (768 X 700, 375 X 812)', () => {
    ...
    viewports.forEach(size => {
      ...
      describe(`Shopping utilities tests on viewport [${checkSizes(size)}]`, () => {
        ...
        it('shows sidebar filter', function () {
          switchViewports(size);
          actions.openFilterNavigationSideBar();
          expects.filterElementsArePresent();
  ...
  describe('Iphone Viewport Tests (375 * 812)', () => {
    ...
    context(`can show navigation bar items on viewport: [${checkSizes(size)}]`, () => {
      it(`displays search icon`, function () {
        shouldBeVisible(fetchTestTitle(this), elements.searchIconIphone(), viewPortSize(size));
      });
      it(`should not display wishlist icon`, function () {
        shouldNotBeVisible(fetchTestTitle(this), elements.wishListIconNavbar(), viewPortSize(size));
      });
      it(`should not display search input`, function () {
        shouldNotBeVisible(fetchTestTitle(this), elements.searchInput(), viewPortSize(size));
      });
      it(`should not display shopping cart item count`, function () {
        shouldNotBeVisible(fetchTestTitle(this), elements.shoppingCartItemCount(), viewPortSize(size));
      });
      it(`should not display desktop filters`, function () {
        shouldNotBeVisible(fetchTestTitle(this), elements.gridFilter(), viewPortSize(size));
        shouldNotBeVisible(fetchTestTitle(this), elements.listFilter(), viewPortSize(size));
      });
    });

    describe(`Footer items tests on click on viewport: [${checkSizes(size)}]`, () => {
      it.skip(`can check whether dropdowns on footer items are closed or opened`, function () {
        // Currently unable to verify this functionality using automated functional tests, can be achievable using visual regression
      });
