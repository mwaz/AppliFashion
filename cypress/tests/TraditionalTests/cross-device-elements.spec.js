...
context('Cross-Device Elements Test', () => {
  ...
  afterEach(
     function() {
      if ( this.currentTest && this.currentTest.state !== undefined ) {
        hackathonReporter(1, this.currentTest.title, elements.logo(), `${checkSizes(this.currentTest.size)}`, viewPortSize(this.size), this.currentTest.state);
      }
  ...
  describe('All Viewport Tests', () => {
    ...
    viewports.forEach(size => {
      ...
      describe.only(`Viewport verification tests on viewport: [${checkSizes(size)}]`, function(){
        it.only(`Displays logo`, async function(){
          switchViewports(size);
          shouldBeVisible(fetchTestTitle(this), elements.logo(), viewPortSize(size));
        });
        it.only(`Displays the top banner`, function(){
          switchViewports(size);
